      ...
      if (Randtheme) {
        ...
        for (n in 1:20) {
          if ((dataK$LOW[n] >= dataK$LOW[n + 1] & dataK$HIGH[n] <= dataK$HIGH[n + 1] & dataK$OPEN[n] <= dataK$CLOSE[n]) | 
            ...
            dot.dat.n = data.frame(xAxis = dataK$Date[n], value = 1, name = 'up', yAxis = dataK$HIGH[n] * 1.04)
            dot.dat = rbind.data.frame(dot.dat, dot.dat.n)
                     ...
                     dataK$LOW[n] > dataK$LOW[n + 1] & dataK$HIGH[n] < dataK$HIGH[n + 1] & dataK$OPEN[n] > dataK$CLOSE[n]) {
            dot.dat.n = data.frame(xAxis = dataK$Date[n], value = -1, name = 'down', yAxis = dataK$LOW[n] * 0.96)
            dot.dat = rbind.data.frame(dot.dat, dot.dat.n)
          }
        }
      }
    ...
    if (dim(bandRes) >= c(1, 1)) {
      ...
      if (Randtheme) {
        Kplot <- Kplot %>% addML(data = K.dat, symbolSize = c(1, 1), mode = 'add', 
                                 effect = list(show = T,color = 'blue',loop = TRUE, period = 60, scaleSize = 1))
        if (length(dot.dat) > 1) {
          Kplot <- Kplot %>% addMP(data = dot.dat, symbol = 'pin',symbolSize =3,effect = list(show = F,color='green'))
        }
      }
      return(Kplot)
    }
...
#### [300177](#300177); [603266](#603266); [300658](#300658); [601689](#601689); [603686](#603686); [603363](#603363); [002577](#002577); [300592](#300592)
...
Note: reports updated on 2020-07-01