...
SpyKplot = function(Scode = '', type = c('sting'), bandRes = bandTest, Protest = ProRes[['Buydat']], viewNum = 60) {
  ...
  if (Scode %in% BuyScodeNum) {
    ...
    Kplot <-  echartr(dataK, as.character(Date), c('OPEN', 'CLOSE', 'LOW', 'HIGH'), type='k') %>%  
      ...
      set_theme(width = 1000,height = 360) %>%  set_legend(show = F)%>% 
  ...
  if (length(Scodes) > 0) {
    ...
    for (sto in Scodes) {
      ...
      if (type == 'hist') {
        ...
        start.date = tradeRe[[sto]]$date[1]
        start.price = tradeRe[[sto]]$price[1]
        start.hday = tradeRe[[sto]]$hday[1]
        start.profit = tradeRe[[sto]]$profit[1]
      } else if (type == 'hold') {
        # get start trade info
      ...
      # calculate new vars
      diffDays = which(dataK$Date == start.date)
      if (diffDays > Maxdays & type == 'hist') {
      ...
      sto.dat.i = data.frame(Scode = sto, Hdays = start.hday, Profit = start.profit, 
                             StartPri = start.price, EndPri = end.price, HighPer = UpPer, LowPer = DownPer, 
                             diffDays = diffDays, HP.pos = diffDays - dHP.pos, LP.pos = diffDays - dLP.pos, 
                             Symbol = Symbol)
      ...
      value1 = diffDays - Second.pos
      value2 = diffDays - Third.pos
